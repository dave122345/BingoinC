	...
	while (Mainmenuopt != 0)
	{
		...
		while (option != 0)
		{
			...
			for (p = 0; p < Numofplayers; p++)
			{
				...
				Line_win(players[p], drawn, 0);
				Line_win(players[p], drawn, 1);
				Line_win(players[p], drawn, 2);
				Line_win2(players[p], drawn, 0, 1);
				Line_win2(players[p], drawn, 1, 2);
				Line_win2(players[p], drawn, 0, 2);
			}//eo for winnings
			...
			if (option == 3)
			{
				...
				for (int p = 0; p < Numofplayers; p++)
				{
					fwrite(players[p], sizeof(struct bingo_player), 1, f);
				}
				fclose(f);
			}
			//menu
